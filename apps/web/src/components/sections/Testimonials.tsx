...
/* -------------------------------------------------------------------------- *
 *  Testimonials come from the CMS.
 *
 *  `pending` travels with each one and still drives the "Placeholder" badge:
 *  a quote is demo copy until it has written client approval, and the badge is
 *  what keeps that visible on the page rather than only in the database.
 * -------------------------------------------------------------------------- */
...
export default function Testimonials({ testimonials: TESTIMONIALS }: TestimonialsProps) {
  ...
  /* Guarded against an out-of-range index: the set comes from the CMS now, so
     it can shrink between renders while the carousel is part-way through it. */
  const active = TESTIMONIALS[Math.min(i, Math.max(0, COUNT - 1))];

  const go = useCallback(
    (d: -1 | 1) => setI((v) => (COUNT === 0 ? 0 : (v + d + COUNT) % COUNT)),
    [COUNT],
  );
  ...
  /* Placed below the hooks, not above them. An early return before a hook
     changes how many hooks run between renders, which React treats as a
     mismatch — the section rendering nothing is correct, doing it earlier is
     not. */
            ...
            <div className="flex items-center pl-8">
              {TESTIMONIALS.map((t, idx) => {
                ...
                return (
                  ...
                  >
                    {/* A portrait only where one exists. An empty `src` is not
                        a blank image — the browser re-requests the current
                        page as the image, which is a wasted round trip on
                        every render. Without a portrait the card falls back to
                    ...
                    {t.image ? (
                      ...
                      <span
                        aria-hidden
                        className="font-mono text-sm uppercase tracking-[0.08em] text-ink-400"
                      >
                        {t.author
                          .split(/\s+/)
                          .filter(Boolean)
