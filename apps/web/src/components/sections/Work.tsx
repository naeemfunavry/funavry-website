...
export default function Work({ caseStudies: PROJECTS }: { caseStudies: CaseStudy[] }) {
  const N = PROJECTS.length;

  /* Rendered twice so the deck loops without a seam: advancing past the last
     card scrolls forward into the copy, then resets to the matching real card
     with no animation. Going back before the first does the mirror of that.
     Derived here rather than at module load — the set comes from the CMS. */
  const SLIDES = useMemo(() => [...PROJECTS, ...PROJECTS], [PROJECTS]);
