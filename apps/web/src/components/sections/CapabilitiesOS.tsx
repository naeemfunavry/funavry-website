...
function Drawer({
  ...
  service: Service;
  /** How many practices there are in total — the copy reads "the other N−1". */
...
export interface CapabilitiesOSProps {
  services: Service[];
}
...
export default function CapabilitiesOS({ services }: CapabilitiesOSProps) {
  ...
  const measure = useCallback(() => {
    ...
    /* TECH and GBS are memoised on `services`, so this only re-measures when
       the practice set actually changes — not on every render. */
  }, [TECH, GBS]);
