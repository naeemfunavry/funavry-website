...
export default function Insights({ posts }: InsightsProps) {
  /* Derived per render rather than at module load: the posts come from the CMS
     now, so which one is featured is a property of the data, not of this file. */
  const featured = posts.find((p) => p.featured) ?? posts[0];
  const rest = posts.filter((p) => p !== featured);
  const [caseTile, leadTile, newsTile] = rest;

  /* The bento is built around four distinct tiles. Below that it has nothing
     to lay out, so the section simply does not render — an empty bento frame
     is worse than no section. */
  if (!featured || !caseTile || !leadTile || !newsTile) return null;
