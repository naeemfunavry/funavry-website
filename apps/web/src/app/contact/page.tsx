...
const statsFor = (industryCount: number) => [
...
export default async function ContactPage() {
  const [chrome, caseStudies] = await Promise.all([getChrome(), getCaseStudies()]);

  const featuredWork = caseStudies.filter((c) => c.featured);
  const STATS = statsFor(chrome.industries.length);
  ...
  return (
    <>
      ...
      <main id="main">
        ...
        <SelectedWork caseStudies={featuredWork} />
