...
import { industriesForService, neighbours, projectsFor } from "@/lib/relations";
...
export default async function ServiceDetailPage({
  ...
  const work = projectsFor(projects, found.caseStudySlugs).slice(0, 6);
  ...
  /* Neighbours are resolved by slug rather than by object identity — the
     service here came from its own fetch, so it is a different object from the
     one in the chrome's list even when it is the same practice. */
  const current = SERVICES.find((s) => s.slug === service.slug) ?? service;
  const { prev, next } = neighbours(SERVICES, current);

  const samePhase = SERVICES.filter(
    (s) => s.phase === service.phase && s.slug !== service.slug,
  );
