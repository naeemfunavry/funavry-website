 ...
 *
 * Built per render now that the social profiles live in the CMS — `sameAs` is
 * the list of profiles Google uses to connect the entity, so it has to reflect
 * what the footer actually links to rather than a snapshot taken at build time.
 */
const buildOrgJsonLd = (socials: { href: string }[]) => ({
...
export default async function RootLayout({
  ...
  const orgJsonLd = buildOrgJsonLd(await getSocials());
